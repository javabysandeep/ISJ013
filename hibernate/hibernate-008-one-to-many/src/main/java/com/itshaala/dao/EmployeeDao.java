...
public class EmployeeDao {
    public void addEmployee(Employee employee) {
        ...
        session.persist(employee.getAddresses().get(0));
        session.persist(employee.getAddresses().get(1));
        transaction.commit();
        session.close();
    ...
    public void updateEmployee(Employee employee) {
        ...
        Transaction transaction = session.beginTransaction();
        Employee fetchedEmployee = session.find(Employee.class, employee.getEmployeeId());
